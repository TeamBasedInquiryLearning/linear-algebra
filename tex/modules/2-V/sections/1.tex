...
\begin{activity}{20}
  ...
  Show that $V$ contains an additive identy element, i.e. there is an element \(\vect{z} \in V\) 
  such that for any \(\vect{v}\in V\), \(\vect{v}\oplus\vect{z}=\vect{v}\)
  \end{subactivity}
\end{activity}
...
\begin{fact}
  ...
  \(y=e^x\). Let \(V=\setBuilder{(x,y)}{y=e^x}\) with operations defined by
  \[
    (x,y)\oplus (z,w)=(x+z,yw)
      \hspace{3em}
    c\odot (x,y)=(cx,y^c)
  ...
  It turns out \(V\) satisifes all eight properties.
    \begin{multicols}{2}
    \begin{itemize}
...
\begin{activity}{15}
  ...
  \begin{subactivity}
    ...
    showing that there is no vector \(\vect{z}=(z_1,z_2)\) for which \((0,-1)\oplus\vec z=(0,-1)\).
  \end{subactivity}

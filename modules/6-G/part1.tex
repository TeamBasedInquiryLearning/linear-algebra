...
\draw[dashed,blue,->] (1,2) -- (4,3);
\draw[dashed,blue,->] (3,1) -- (4,3);
\draw[thick,red,->] (0,0) -- node[above left] {$\vec{w}+c\vec{v}$} (5.5,3.5);
\draw[dashed,red,->] (3,1) -- (8.5,4.5);
\draw[dashed,red,->] (5.5,3.5) -- (8.5,4.5);
\draw[thin,dashed,gray] (3,1) -- (2.5,2.5);
\draw[thin,dashed,gray] (4,3) -- (5.5,3.5);
\end{tikzpicture}
\end{center}

The base of both parallelograms is $\vec{v}$, while the height has not changed.  Thus
$$\det ([\vec{v}\hspace{1em}\vec{w}+c\vec{v}]) = \det ([\vec{v} \hspace{1em} \vec{w}])$$
\end{observation}

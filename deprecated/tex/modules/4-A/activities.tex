...
standard matrix
\[T\left( \begin{bmatrix} x \\ y \\ z \end{bmatrix} = \begin{bmatrix} 3x+4y-z \\ x+2y+z \end{bmatrix}\right)\]
\begin{subactivity}
Set
\(
  T\left(\begin{bmatrix}x\\y\\z\end{bmatrix}\right)
...
\begin{enumerate}[(a)]
\item The number of pivot columns
\item The number of non-pivot columns
\item The number of pivot rows
\item The number of non-pivot rows
\end{enumerate}
\end{activity}
...
\begin{observation}
Combining these with the observation that the number of columns is the dimension of the domain of \(T\), we have the \term{rank-nullity theorem}:
\vfill
\begin{minipage}{0.8\textwidth}
The dimension of the domain of \(T\) equals \(\dim(\ker T)+\dim(\Im T)\).
\end{minipage}
\vfill
The dimension of the image is called the \term{rank} of \(T\) (or \(A\)) and the dimension of the kernel is called the \term{nullity}. 
\end{observation}
